...
export default function EditPost() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const rawPost = await api.getPost(id);
        if (!rawPost || rawPost.error) return;
        const aid = rawPost.authorId?._id || rawPost.authorId;
        const post = { ...rawPost, id: rawPost._id, authorId: aid };

        // Check access: must be owner or have an approved edit request
        const isOwner = !post.authorId || post.authorId === user?.id;
        if (!isOwner) {
          try {
            const editStatus = await api.getEditRequestStatus(id);
            if (!editStatus || editStatus.status !== "approved") {
        ...
        setAuthorTwitter(post.socials?.twitter || "");
        setAuthorGithub(post.socials?.github || "");
        setAuthorWebsite(post.socials?.website || "");
        if (editorRef.current) {
  ...
  return (
    <div className="editor-page">
      ...
      <div className="editor-actions">
        <button className="btn btn-primary btn-lg" onClick={handleUpdate} disabled={!title.trim()}>
